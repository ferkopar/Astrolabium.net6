...
namespace Astrolabium.Services
{
    ...
    public class CalcService : ICalcService
    {
        ...
        private static EphemerisResult SwephCalculation(Configuration config, InputCalculation input) {
            ...
            // Initialize paths
            var paths = config.SearchPaths.Where(s => !string.IsNullOrWhiteSpace(s)).Union(new[] { "." });
            ...
            using (var sweph = new Sweph()) {
                ...
                sweph.swe_calc(result.EphemerisTime, Planet.EclipticNutation, x, ref serr);
                ...
                foreach (var planet in input.Planets) {
                    var p = new SweCore.Planets.Planet(planet.Id);
                    ...
                    if (planet == Planet.FixedStar) {
                        ...
                        //    pi.PlanetName = $"#{planet - Planet.FirstAsteroid}";
                        //}
                    }
                    if (iflgret >= 0) {
                        pi.Longitude = x[0];
                        ...
                        if (Math.Abs(pi.HousePosition) < .0001)
                        {
                ...
                // Houses
                var cusps = new double[input.HouseSystem == HouseSystem.GauquelinSector ? 37 : 13];
                var ascmc = new double[10];
                var hNames = new[] { "", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI", "XII" };
                var amNames = new[] { "Ascendant", "MC", "ARMC", "Vertex", "Equatorial ascendant", 
                ...
                foreach (var p in result.Planets)
                {
                    p.SetHouse(result.Houses);
                }
                for (var i = 0; i < 7; i++) {
