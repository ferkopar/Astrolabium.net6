...
namespace SwissEphCore.CPort
{
    ...
    class SwemMoon:BaseCPort
    {
        ...
        */
        ...
        //const bool MOSH_MOON_200 = false;   // Is not defined in C source
...
/*
 ...
 4, 1,-1,-1,    0,-1719,
         ...
         */
        // rename with prefix moon_ because conflict avec sweplan
        double[,] moon_ss = new double[5, 8];
        double[,] moon_cc = new double[5, 8];
         ...
         */
        public int swi_moshmoon(double tjd, bool do_save, CPointer<double> xpmret, ref string serr) {
            int i;
            double a, b, t;
            double[] x1 = new double[6], x2 = new double[6];
            double[] xx = new double[6], xpm;
            Sweph.plan_data pdp = SE.Sweph.swed.pldat[Sweph.SEI_MOON];
            ...
            if (tjd < Sweph.MOSHLUEPH_START - 0.2 || tjd > Sweph.MOSHLUEPH_END + 0.2) {
                s = C.sprintf("jd %f outside Moshier's Moon range %.2f .. %.2f ",
                      tjd, Sweph.MOSHLUEPH_START, Sweph.MOSHLUEPH_END);
        ...
        void moon1() {
            ...
            moon_sscc(0, Sweph.STR * D, 6);
            moon_sscc(1, Sweph.STR * M, 4);
            moon_sscc(2, Sweph.STR * MP, 4);
            moon_sscc(3, Sweph.STR * NF, 4);
            moonpol[0] = 0.0;
            moonpol[1] = 0.0;
            moonpol[2] = 0.0;
            /* terms in T^2, scale 1.0 = 10^-5" */
            ...
            moonpol[2] += 5.01 * cg + 2.72 * sg;
            g = Sweph.STR * (10.0 * Ve - 3.0 * Ea - MP);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += -0.253102 * cg + 0.503359 * sg;
            l1 += 1258.46 * cg + 707.29 * sg;
            l2 += z[14] * cg + z[15] * sg;
            g = Sweph.STR * (8.0 * Ve - 13.0 * Ea);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += -0.187231 * cg - 0.127481 * sg;
            l1 += -319.87 * cg - 18.34 * sg;
            ...
            l += -0.165009 * cg + 0.044176 * sg;
            l1 += 4.67 * cg + 201.55 * sg;
            g = Sweph.STR * f;  /* 18V - 16E */
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += 0.330401 * cg + 0.661362 * sg;
            l1 += 1202.67 * cg - 555.59 * sg;
            l2 += z[20] * cg + z[21] * sg;
            g = Sweph.STR * (f - 2.0 * MP);  /* 18V - 16E - 2l */
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += 0.352185 * cg + 0.705041 * sg;
            l1 += 1283.59 * cg - 586.43 * sg;
            g = Sweph.STR * (2.0 * Ju - 5.0 * Sa);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += -0.034700 * cg + 0.160041 * sg;
            l2 += z[22] * cg + z[23] * sg;
            g = Sweph.STR * (SWELP - NF);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += 0.000116 * cg + 7.063040 * sg;
            l1 += 298.8 * sg;
            /* T^3 terms */
            sg = Math.Sin(Sweph.STR * M);
            ...
            chewm(BT, NBT, 4, 4, moonpol);
            chewm(LRT, NLRT, 4, 1, moonpol);
            g = Sweph.STR * (f - MP - NF - 2355767.6); /* 18V - 16E - l - F */
            moonpol[1] += -1127.0 * Math.Sin(g);
            g = Sweph.STR * (f - MP + NF - 235353.6); /* 18V - 16E - l + F */
            moonpol[1] += -1123.0 * Math.Sin(g);
            g = Sweph.STR * (Ea + D + 51987.6);
            moonpol[1] += 1303.0 * Math.Sin(g);
            g = Sweph.STR * SWELP;
            moonpol[1] += 342.0 * Math.Sin(g);
            g = Sweph.STR * (2.0 * Ve - 3.0 * Ea);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += -0.343550 * cg - 0.000276 * sg;
            l1 += 105.90 * cg + 336.53 * sg;
            g = Sweph.STR * (f - 2.0 * D); /* 18V - 16E - 2D */
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += 0.074668 * cg + 0.149501 * sg;
            l1 += 271.77 * cg - 124.20 * sg;
            g = Sweph.STR * (f - 2.0 * D - MP);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += 0.073444 * cg + 0.147094 * sg;
            l1 += 265.24 * cg - 121.16 * sg;
            ...
            l += 0.288209 * cg - 0.025901 * sg;
            l1 += -63.51 * cg - 240.14 * sg;
            g = Sweph.STR * (2.0 * Ea - 3.0 * Ju + 2.0 * D - MP);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += 0.077865 * cg + 0.438460 * sg;
            l1 += 210.57 * cg + 124.84 * sg;
            g = Sweph.STR * (Ea - 2.0 * Ma);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += -0.216579 * cg + 0.241702 * sg;
            l1 += 197.67 * cg + 125.23 * sg;
            ...
            l += -0.133430 * cg + 0.041079 * sg;
            l1 += 6.28 * cg + 169.08 * sg;
            g = Sweph.STR * (3.0 * Ve - 4.0 * Ea);
            cg = Math.Cos(g);
            sg = Math.Sin(g);
            l += -0.175074 * cg + 0.003035 * sg;
            l1 += 49.17 * cg + 150.57 * sg;
            g = Sweph.STR * (2.0 * (Ea + D - MP) - 3.0 * Ju + 213534.0);
            l1 += 158.4 * Math.Sin(g);
            l1 += moonpol[0];
            a = 0.1 * T; /* set amplitude scale of 1.0 = 10^-4 arcsec */
            moonpol[1] *= a;
        ...
        void moon2() {
            /* terms in T^0 */
            g = Sweph.STR * (2 * (Ea - Ju + D) - MP + 648431.172);
            l += 1.14307 * Math.Sin(g);
            g = Sweph.STR * (Ve - Ea + 648035.568);
            l += 0.82155 * Math.Sin(g);
            g = Sweph.STR * (3 * (Ve - Ea) + 2 * D - MP + 647933.184);
            l += 0.64371 * Math.Sin(g);
            g = Sweph.STR * (Ea - Ju + 4424.04);
            l += 0.63880 * Math.Sin(g);
            g = Sweph.STR * (SWELP + MP - NF + 4.68);
            l += 0.49331 * Math.Sin(g);
            g = Sweph.STR * (SWELP - MP - NF + 4.68);
            l += 0.4914 * Math.Sin(g);
            g = Sweph.STR * (SWELP + NF + 2.52);
            l += 0.36061 * Math.Sin(g);
            g = Sweph.STR * (2.0 * Ve - 2.0 * Ea + 736.2);
            l += 0.30154 * Math.Sin(g);
            g = Sweph.STR * (2.0 * Ea - 3.0 * Ju + 2.0 * D - 2.0 * MP + 36138.2);
            l += 0.28282 * Math.Sin(g);
            g = Sweph.STR * (2.0 * Ea - 2.0 * Ju + 2.0 * D - 2.0 * MP + 311.0);
            l += 0.24516 * Math.Sin(g);
            g = Sweph.STR * (Ea - Ju - 2.0 * D + MP + 6275.88);
            l += 0.21117 * Math.Sin(g);
            g = Sweph.STR * (2.0 * (Ea - Ma) - 846.36);
            l += 0.19444 * Math.Sin(g);
            g = Sweph.STR * (2.0 * (Ea - Ju) + 1569.96);
            l -= 0.18457 * Math.Sin(g);
            g = Sweph.STR * (2.0 * (Ea - Ju) - MP - 55.8);
            l += 0.18256 * Math.Sin(g);
            g = Sweph.STR * (Ea - Ju - 2.0 * D + 6490.08);
            l += 0.16499 * Math.Sin(g);
            g = Sweph.STR * (Ea - 2.0 * Ju - 212378.4);
            l += 0.16427 * Math.Sin(g);
            g = Sweph.STR * (2.0 * (Ve - Ea - D) + MP + 1122.48);
            l += 0.16088 * Math.Sin(g);
            g = Sweph.STR * (Ve - Ea - MP + 32.04);
            l -= 0.15350 * Math.Sin(g);
            g = Sweph.STR * (Ea - Ju - MP + 4488.88);
            l += 0.14346 * Math.Sin(g);
            g = Sweph.STR * (2.0 * (Ve - Ea + D) - MP - 8.64);
            l += 0.13594 * Math.Sin(g);
            g = Sweph.STR * (2.0 * (Ve - Ea - D) + 1319.76);
            l += 0.13432 * Math.Sin(g);
            g = Sweph.STR * (Ve - Ea - 2.0 * D + MP - 56.16);
            l -= 0.13122 * Math.Sin(g);
            g = Sweph.STR * (Ve - Ea + MP + 54.36);
            l -= 0.12722 * Math.Sin(g);
            g = Sweph.STR * (3.0 * (Ve - Ea) - MP + 433.8);
            l += 0.12539 * Math.Sin(g);
            g = Sweph.STR * (Ea - Ju + MP + 4002.12);
            l += 0.10994 * Math.Sin(g);
            g = Sweph.STR * (20.0 * Ve - 21.0 * Ea - 2.0 * D + MP - 317511.72);
            l += 0.10652 * Math.Sin(g);
            g = Sweph.STR * (26.0 * Ve - 29.0 * Ea - MP + 270002.52);
            l += 0.10490 * Math.Sin(g);
            g = Sweph.STR * (3.0 * Ve - 4.0 * Ea + D - MP - 322765.56);
            l += 0.10386 * Math.Sin(g);
            g = Sweph.STR * (SWELP + 648002.556);
            B = 8.04508 * Math.Sin(g);
            g = Sweph.STR * (Ea + D + 996048.252);
            B += 1.51021 * Math.Sin(g);
            g = Sweph.STR * (f - MP + NF + 95554.332);
            B += 0.63037 * Math.Sin(g);
            g = Sweph.STR * (f - MP - NF + 95553.792);
            B += 0.63014 * Math.Sin(g);
            g = Sweph.STR * (SWELP - MP + 2.9);
            B += 0.45587 * Math.Sin(g);
            g = Sweph.STR * (SWELP + MP + 2.5);
            B += -0.41573 * Math.Sin(g);
            g = Sweph.STR * (SWELP - 2.0 * NF + 3.2);
            B += 0.32623 * Math.Sin(g);
            g = Sweph.STR * (SWELP - 2.0 * D + 2.5);
         ...
         */
        void moon4() {
            moonpol[2] /= Sweph.AUNIT / 1000;
            moonpol[0] = Sweph.STR * mods3600(moonpol[0]);
            moonpol[1] = Sweph.STR * moonpol[1];
            B = moonpol[1];
        }

        const double CORR_MNODE_JD_T0GREG = -3063616.5;   /* 1 jan -13100 greg. */
        const double CORR_MNODE_JD_T1GREG = 844477.5;   /* 1 jan  -2400 jul. */
        const double CORR_MNODE_JD_T2GREG = 2780263.5;   /* 1 jan   2900 jul. */
        const double CORR_MNODE_JD_T3GREG = 7930182.5;   /* 1 jan  17000 greg. */
        double corr_mean_node(double J) {
            ...
            if (J < Sweph.JPL_DE431_START) return 0;
            if (J > Sweph.JPL_DE431_END) return 0;
         ...
         */
        public int swi_mean_node(double J, CPointer<double> pol, ref string serr) {
            //#if 0
            //  double a, b, c;
            //#endif
            string s = string.Empty;
            ...
            /* longitude */
            pol[0] = SE.SwephLib.swi_mod2PI((SWELP - NF - dcor) * Sweph.STR);
            /* latitude */
            pol[1] = 0.0;
            /* distance */
            pol[2] = Sweph.MOON_MEAN_DIST / Sweph.AUNIT; /* or should it be derived from mean
                        * orbital ellipse? */
            //#if 0
            //  a = pol[0];
        ...
        const double CORR_MAPOG_JD_T0GREG = -3063616.5;   /* 1 jan -13100 greg. */
        const double CORR_MAPOG_JD_T1GREG = 1209720.5;   /* 1 jan  -1400 greg. */
        const double CORR_MAPOG_JD_T2GREG = 2780263.5;   /* 1 jan   2900 greg. */
        const double CORR_MAPOG_JD_T3GREG = 7930182.5;   /* 1 jan  17000 greg. */
        double corr_mean_apog(double J) {
            double J0, dJ, dayscty, dcor, dcor0, dcor1, dfrac;
            int i;
         ...
         */
        public int swi_mean_apog(double J, CPointer<double> pol, ref string serr) {
            //#if 0
            //  int i;
            //  double a, b;
            //  double x[3];
            ...
            /* with elements from swi_moshmoon2(), which are fitted to jpl-ephemeris */
            if (J < Sweph.MOSHNDEPH_START || J > Sweph.MOSHNDEPH_END) {
                s = C.sprintf("jd %f outside mean apogee range %.2f .. %.2f ",
                       J, Sweph.MOSHNDEPH_START, Sweph.MOSHNDEPH_END);
                serr = (serr ?? String.Empty) + s;
                return (Sweph.ERR);
            }
            mean_elements();
            pol[0] = SE.SwephLib.swi_mod2PI((SWELP - MP) * Sweph.STR + Math.PI);
            pol[1] = 0;
             ...
             */
            dcor = corr_mean_apog(J) * SwissEph.DEGTORAD;
            pol[0] = SE.SwephLib.swi_mod2PI(pol[0] - dcor);
            /* apogee is now projected onto ecliptic */
            node = (SWELP - NF) * Sweph.STR;
            dcor = corr_mean_node(J) * SwissEph.DEGTORAD;
            node = SE.SwephLib.swi_mod2PI(node - dcor);
            pol[0] = SE.SwephLib.swi_mod2PI(pol[0] - node);
            SE.SwephLib.swi_polcart(pol, pol);
            SE.SwephLib.swi_coortrf(pol, pol, -Sweph.MOON_MEAN_INCL * SwissEph.DEGTORAD);
            SE.SwephLib.swi_cartpol(pol, pol);
            pol[0] = SE.SwephLib.swi_mod2PI(pol[0] + node);
            return SwissEph.OK;
        }
         ...
         */
        void chewm(CPointer<short> pt, int nlines, int nangles, int typflg, CPointer<double> ans) {
            int i, j, k, k1, m;
        ...
        void ecldat_equ2000(double tjd, CPointer<double> xpm) {
            /* cartesian */
            SE.SwephLib.swi_polcart(xpm, xpm);
            /* equatorial */
            SE.SwephLib.swi_coortrf2(xpm, xpm, -SE.Sweph.swed.oec.seps, SE.Sweph.swed.oec.ceps);
            /* j2000 */
            SE.SwephLib.swi_precess(xpm, tjd, 0, Sweph.J_TO_J2000);/**/
        }
        ...
        public void swi_mean_lunar_elements(double tjd,
            ...
            T2 = T * T;
            mean_elements();
            node = SE.swe_degnorm((SWELP - NF) * Sweph.STR * SwissEph.RADTODEG);
            peri = SE.swe_degnorm((SWELP - MP) * Sweph.STR * SwissEph.RADTODEG);
            T -= 1.0 / 36525;
            mean_elements();
            dnode = SE.swe_degnorm(node - (SWELP - NF) * Sweph.STR * SwissEph.RADTODEG);
            dnode -= 360;
            dperi = SE.swe_degnorm(peri - (SWELP - MP) * Sweph.STR * SwissEph.RADTODEG);
            dcor = corr_mean_node(tjd);
            node = SE.swe_degnorm(node - dcor);
            dcor = corr_mean_apog(tjd);
            peri = SE.swe_degnorm(peri - dcor);
        }
         ...
         */
        public int swi_intp_apsides(double J, CPointer<double> pol, int ipli) {
            double dd;
            ...
            sLP = mods3600(SWELP);
            sMP = mods3600(MP);
            if (ipli == Sweph.SEI_INTP_PERG) { MP = 0.0; niter = 5; }
            if (ipli == Sweph.SEI_INTP_APOG) { MP = 648000.0; niter = 4; }
            cMP = 0;
            dd = 18000.0;
